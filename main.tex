...
\usepackage{tcolorbox} % Per caselle di testo personalizzate
\usepackage{multirow}
...
% Configurazione tcolorbox per centrare e stilizzare la casella
\tcbuselibrary{minted, listings, skins}
\tcbset{beamer, minted options={fontsize=\footnotesize, frame=none}}

\newtcbox{\centeredminted}[2][]{%
  enhanced,
  colframe=blue!70!black, colback=blue!5, sharp corners,
  fonttitle=\bfseries,
...
\begin{frame}{Einstein Trust Layer}
  ...
  \only<4>{
    \begin{center}
      \centeredminted[]{text}{
       $\text{Patient }\texttt{Zanolin Lorenzo} \ldots \rightarrow \text{Patient }\texttt{PERSON\_0} \ldots$
      }
    \end{center}      
  }
...
\begin{frame}{Patient Summary}
  ...
  \uncover<1->{
    Invoked by the doctor before receiving the patient: quick and detailed overview of all the patient’s conditions.
    \vfill
  }
  ...
  \only<2>{
  \begin{figure}[]
    \includegraphics[scale=0.35]{images/SummaryDetail.png}
  \end{figure}
  \vfill
  }
\end{frame}
...
\begin{frame}{List Possible Problems}
  ...
  % Testo statico in alto
  \uncover<1->{
    \vspace{-0.5cm} % Spostamento verso l'alto per il posizionamento preciso
    Identifies potential conditions based on the patient’s symptoms, providing a list of possible diagnoses and medications, with dosage calculations for all of them.
  }
  
  % Spazio verticale tra testo e immagini
  \vspace{0.5cm}
  
  % Immagini con transizione
  \only<1>{
    \begin{center}
      \includegraphics[width=\textwidth,keepaspectratio]{images/ListPossibleProblems.png}
    \end{center}
  }
  \only<2>{
    \begin{center}
      \includegraphics[width=0.8\textwidth,keepaspectratio]{images/ProblemsDetail.png}
    \end{center}
  }
